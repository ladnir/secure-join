...
void report(const Options& o,const Protocol& p,const Measurement& t) {
    ...
    std::cout<<std::fixed<<std::setprecision(3)<<"{\"type\":\""<<(o.audit?"round_audit":"benchmark")<<"\",\"method\":\""<<o.method
        ...
        <<",\"real_crypto\":true,\"verified\":true,\"os_threads\":1,\"seed\":"<<o.seed<<",\"batch_size\":"<<o.batch
...
// Both outbound vectors are captured before either is delivered. Every wave
// therefore contains only messages independent of receives in that same wave.
template<class T>u64 auditDrive(std::array<coproto::BufferingSocket,2>& s,T task) {
    auto running=macoro::make_blocking(std::move(task));u64 rounds=0;
    for(;;) {
        auto a=s[0].getOutbound(),b=s[1].getOutbound();
        if(!a||!b)throw std::runtime_error("Closed audit transport");
        if(a->empty()&&b->empty()) {
...
template<class S>void local(const Options& o,S sockets) {
    ...
    for(u64 i=0;i<x[0].size();++i)x[0](i)^=x[1](i);for(u64 i=0;i<y[0].size();++i)y[0](i)^=y[1](i);
    CorGenerator c[2];Protocol p[2];AdditivePerm out[2];Measurement t;auto start=Clock::now();
    for(int i=0;i<2;++i){c[i].init(sockets[i].fork(),prng[i],i,o.concurrency,o.batch,false);p[i].init(o,c[i]);}
    auto off=Clock::now();u64 before=sockets[0].bytesSent()+sockets[1].bytesSent();
    ...
    if constexpr(std::is_same_v<S,std::array<coproto::BufferingSocket,2>>)t.offRounds=auditDrive(sockets,std::move(prepare));
    else {auto r=macoro::sync_wait(std::move(prepare));std::apply([](auto&... q){(q.result(),...);},r);}
    {auto r=macoro::sync_wait(macoro::when_all_ready(sockets[0].flush(),sockets[1].flush()));std::get<0>(r).result();std::get<1>(r).result();}
    auto on=Clock::now();u64 prepared=sockets[0].bytesSent()+sockets[1].bytesSent();
    ...
    if constexpr(std::is_same_v<S,std::array<coproto::BufferingSocket,2>>)t.onRounds=auditDrive(sockets,std::move(merge));
    else {auto r=macoro::sync_wait(std::move(merge));std::get<0>(r).result();std::get<1>(r).result();}
    {auto r=macoro::sync_wait(macoro::when_all_ready(sockets[0].flush(),sockets[1].flush()));std::get<0>(r).result();std::get<1>(r).result();}
    auto end=Clock::now();t.setup=ms(start,off);t.offline=ms(off,on);t.online=ms(on,end);t.offBytes=prepared-before;t.onBytes=sockets[0].bytesSent()+sockets[1].bytesSent()-prepared;
    verify(out[0],out[1],d);report(o,p[0],t);
}
macoro::task<> network(const Options& o,boost::asio::io_context& io) {
    auto s=o.party==0?co_await coproto::AsioAcceptor(o.address,io,1):co_await coproto::AsioConnect(o.address,io);
    std::vector<u64> cfg={o.m,o.n,o.bits,o.block,o.base,o.depth,o.batch,o.concurrency,o.seed,o.terminal,o.pivots,o.reserve};
    for(auto v:o.children)cfg.push_back(v);cfg.push_back(~u64(0));for(auto v:o.cubes)cfg.push_back(v);
    ...
    co_await finish(s,s.send(coproto::copy(length)),s.recv(otherLength));if(length!=otherLength)throw std::runtime_error("Configuration length mismatch");
    co_await finish(s,s.send(coproto::copy(cfg)),s.recv(peer));if(cfg!=peer)throw std::runtime_error("Configuration mismatch");
    ...
    c.init(s.fork(),rng,o.party,o.concurrency,o.batch,false);p.init(o,c);
    auto off=Clock::now();u64 before=s.bytesSent();co_await finish(s,c.start(),p.prepare(s,rng));auto offEnd=Clock::now();t.offBytes=s.bytesSent()-before;
    u64 barrier=1,peerBarrier=0;co_await finish(s,s.send(coproto::copy(barrier)),s.recv(peerBarrier));if(peerBarrier!=barrier)throw std::runtime_error("Barrier mismatch");
    auto on=Clock::now();before=s.bytesSent();co_await finish(s,p.merge(x,y,out,s,rng));auto end=Clock::now();t.onBytes=s.bytesSent()-before;
    t.setup=ms(start,off);t.offline=ms(off,offEnd);t.online=ms(on,end);
    barrier=2;co_await finish(s,s.send(coproto::copy(barrier)),s.recv(peerBarrier));if(peerBarrier!=barrier)throw std::runtime_error("Barrier mismatch");
    other.mShare.resize(out.size());co_await finish(s,s.send(coproto::copy(out.mShare)),s.recv(other.mShare));verify(out,other,d);report(o,p,t);
}
int main(int argc,char** argv) {
    try {
        auto o=parse(argc,argv);
        if(o.party<0) {if(o.audit)local(o,std::array<coproto::BufferingSocket,2>{});else local(o,coproto::LocalAsyncSocket::makePair());}
        else {
            // The calling thread executes every protocol and I/O callback.
            // No global coproto context (which starts worker threads) is used.
            boost::asio::io_context io;auto running=macoro::make_blocking(network(o,io));
            io.run();running.get();
        }
        return 0;
