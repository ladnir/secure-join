    ...
    for shape,methods in METHODS.items():
        for runtime in [False,True]:
            ...
            for j,profile in enumerate(PROFILES):
                ...
                axs[0,j].set_title(PNAMES[profile]);axs[1,j].set_xlabel(r'List length $n$')
                for i in range(2):
                    ax=axs[i,j];ax.set_xlim(7.7,20.3);ax.set_xticks([8,12,16,20])
                    ax.set_xticklabels([rf'$2^{{{e}}}$' for e in [8,12,16,20]]);ax.grid(axis='y',alpha=.18)
