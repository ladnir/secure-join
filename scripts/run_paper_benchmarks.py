...
def validate(out):
    ...
    for method in ['logstar','median','cube','sqrt','batcher','quick']:
        ...
        opts=['--method',method,'--m',str(m),'--n',str(n),'--batch-size','16384','--bits','32','--concurrency','2']
        ...
        for phase in ['offline','online']:
            ...
            # Configuration traffic uses a separate, excluded control stream.
            # Check that transport framing does not change the application cost.
...
def main():
    ...
    ap.add_argument('--methods',help='Optional comma-separated methods for an initial stress subset')
    ap.add_argument('--shapes',help='Optional comma-separated shapes for an initial stress subset')
    ...
    for field,requested in [('method',a.methods),('shape',a.shapes)]:
        if requested:
            wanted=set(requested.split(','))
            if not wanted<={c[field] for c in configs}:raise ValueError('Unknown '+field+' selection')
            configs=[c for c in configs if c[field] in wanted]
    if not configs:raise ValueError('Empty experiment selection')
